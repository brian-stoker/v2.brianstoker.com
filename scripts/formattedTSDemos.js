 ...
 */
const ignoreList = ['/pages.ts', 'data/joy/getting-started/templates'];
  ...
  try {
    ...
    if (enableJSXPreview) {
      transformOptions.plugins = transformOptions.plugins.concat([
        [
          require.resolve('src/modules/utils/babel-plugin-jsx-preview'),
...
async function main(argv) {
  ...
  const tsxFiles = [
    ...(await getFiles(path.join(workspaceRoot, 'src/pages'))), // old structure
    ...(await getFiles(path.join(workspaceRoot, 'data'))), // new structure
