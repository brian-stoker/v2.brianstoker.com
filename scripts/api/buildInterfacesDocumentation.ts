...
function generateImportStatement(object: ParsedObject, projects: XTypeScriptProjects) {
  const projectImports = Array.from(projects.values())
    .map((project) => {
      ...
      return `import { ${object.name} } from '@stoked-ui/${project.name}'`;
    })
...
export async function buildInterfacesDocumentationPage(
  ...
  ensureDirSync(apiPagesDirectory);
  ensureDirSync(translationPagesDirectory);
  ensureDirSync(importTranslationPagesDirectory);
  ...
  for (const [interfaceName, packagesWithThisInterface] of Array.from(
    ...
    import InterfaceApiPage from 'src/modules/components/InterfaceApiPage';
    import layoutConfig from 'src/modules/utils/dataGridLayoutConfig';
