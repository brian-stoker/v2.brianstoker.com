...
- 👩‍🎤 We have rolled out the new **style engine** to all the components.
  ...
  In v5, we have standardized on the `styled()` API as the styling foundation we build on top of, and introduced the [the `sx` prop](https://brianstoker.com/system/getting-started/the-sx-prop/) for one-off customizations.
  The `styled()` API is loved by the community, and implemented by a number of styling libraries: styled-components, emotion, stitches, goober, etc. It allows us to support them all with [adapters](https://brianstoker.com/material-ui/integrations/styled-components/).
...
- 💄 We have [updated the style of the Slider](https://brianstoker.com/material-ui/react-slider/#sizes) to better match the Material Design guidelines, and kept a similar style as before under `size="small"`:

  <a href="https://brianstoker.com/material-ui/react-slider/#sizes"><img loading="lazy" src="/static/blog/2021-q2-update/slider.png" alt="" style="width: 838px; margin-bottom: 16px;" /></a>

- ✨ The new style engine has unlocked problems on the `Grid` component that we couldn't solve before with JSS:

  We have added support for [row & column](https://brianstoker.com/material-ui/react-grid/#row-amp-column-spacing) spacing:

```jsx
<Grid container rowSpacing={1} columnSpacing={2} />
```

We have added support for [responsive values](https://brianstoker.com/material-ui/react-grid/#responsive-values) on all the props:

```jsx
<Grid container spacing={{ xs: 2, md: 3 }} />
```

We have added support for a different [number of columns](https://brianstoker.com/material-ui/react-grid/#columns) than 12:

```jsx
<Grid container columns={16}>
```

We have added an alternative implementation that uses [CSS grid](https://brianstoker.com/material-ui/react-grid/#css-grid-layout):
...
- 💄 We have improved the accessibility of the Link component:
  ...
  <a href="https://brianstoker.com/material-ui/react-link/">
...
- ✍️ We have added support for the [cell edit mode](https://brianstoker.com/x/react-data-grid/editing/).
...
In the first quarter, we focused on introducing a [handbook](https://brianstoker.com/blog/2021-q1-update/#handbook) to share our culture and to help solve the N(N-1)/2 communication channels problem that comes with a larger team.
...
- 🦴 Migrate more components to `@mui/base`. [Michał](https://github.com/michaldudak) has recently added support for the [Switch](https://brianstoker.com/base-ui/react-switch/).
